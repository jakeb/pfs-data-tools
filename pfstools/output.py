...
def xlcol(col : 'str | int' ):
    ...
    if isinstance(col, str):
        return sum([ (ord(col[len(col)-i-1]) -64)* 26**i  for i in reversed(range(len(col)))])
    else:
        if col<=26:
            ret = chr(64+col)
        else:
...
def to_excel(
    ...
    columns = (list(df.index.names) if outindex else []) + list(df.columns)        
    df.to_excel(outfile, index=outindex, sheet_name=sheet_name, startrow=start_row, startcol=start_col)
    ...
    if len(df.index) > 0:
        ...
        col_lengths = {
            c: min(
                100, max(10, len(c), floor(df[c].astype(str).map(len).max() * 1.25))
            )
            for c in df
        }

        for c in [ c for c in columns if not c in df ]:
                col_lengths[c] = max(10, len(c))
